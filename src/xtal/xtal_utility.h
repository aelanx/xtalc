...
/// unsigned char
typedef SelectType<sizeof(char)>::uint_t uchar;
...
uint_t string_hashcode(const char* str, uint_t size);
...
struct LongLivedString{
	const char* str() const{ return (char*)this; }
	uint_t size() const{ return string_data_size((char*)this); }
};
...
enum{
	SMALL_STRING_MAX = sizeof(void*) / sizeof(char)
};
