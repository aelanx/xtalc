...
public:
	...
	virtual int_t ch_len(char /*lead*/){ return 1; }
	virtual int_t ch_len2(const char* str){ return ch_len(*str); }
	virtual int_t ch_inc(const char* data, int_t data_size, char* dest, int_t dest_size);
	virtual int_t ch_cmp(const char* a, int_t asize, const char* b, int_t bsize);
};
...
public:
	...
	virtual uint_t mtime(const char*){ return 0; }

	virtual void* new_file_stream(const char* /*path*/, const char* /*flags*/){ return 0; }
	...
	virtual void* new_entries(const char* /*path*/){ return 0; }
	virtual void delete_entries(void* /*entries_object*/){}
	virtual const char* next_entries(void* /*entries_object*/){ return 0; }
...
*
*/
IDPtr intern(const char* str, uint_t size);

/**
* \brief C文字列からsize分の長さを取り出しインターン済み文字列に変換する
*
*/
IDPtr intern(const char* str, uint_t size, String::long_lived_t);
...
*
* int_t ch_len(char lead)が呼ばれた後、マイナスの値を返した場合に続けて呼ぶ。
...
/**
* \brief requireの実行をhookする関数を登録解除する
* \param hook 登録した関数
*/
