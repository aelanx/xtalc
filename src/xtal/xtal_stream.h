...
public:
	...
	* \brief 文字列strをストリームに流す
	*/
	void put_s(const char* str, const char* end){
		put_s(str, end-str);
	}
	...
	* \brief 文字列strをストリームに流す
	*/
	void put_s(const char* str, uint_t length);
...
public:
	...
	*/
	...
	void put_ch_code_be(char ch){
		put_t_be<SelectType<sizeof(char)>::uint_t>(ch);
	}

	char get_ch_code_be(){
		return get_t_be<SelectType<sizeof(char)>::uint_t>();
	}

	void put_ch_code_le(char ch){
		put_t_le<SelectType<sizeof(char)>::uint_t>(ch);
	}

	char get_ch_code_le(){
		return get_t_le<SelectType<sizeof(char)>::uint_t>();
	}
