...
StringPtr Serializer::inner_deserialize_string(int_t charsize, bool bintern){
	...
	if(uint_t sz = stream_->get_u32be()){
		XMallocGuard guard(sizeof(char)*sz);
		char* p = (char*)guard.get();

		switch(charsize){
			XTAL_DEFAULT{ for(uint_t i = 0; i<sz; ++i){ p[i] = (char)stream_->get_u8(); } }
			XTAL_CASE(2){ for(uint_t i = 0; i<sz; ++i){ p[i] = (char)stream_->get_u16be(); } }
			XTAL_CASE(4){ for(uint_t i = 0; i<sz; ++i){ p[i] = (char)stream_->get_u32be(); } }
		}
