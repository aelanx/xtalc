...
void Stream::put_s(const char* str, uint_t length){
	write(str, length*sizeof(char));
}
...
void Stream::skip_bom(){
	...
	char head[1] = {0};
	int_t pos = tell();
	int_t n = read(head, 1*sizeof(char));

	if(n==1*sizeof(char) && (uchar)head[0]==(uchar)0xFEFF || (uchar)head[0]==(uchar)0xFFFE){
		return;
	}	
	...
	char head[3] = {0};
	int_t pos = tell();
	int_t n = read(head, 3*sizeof(char));

	if(n==3*sizeof(char) && (uchar)head[0]==(uchar)239 && (uchar)head[1]==(uchar)187 && (uchar)head[2]==(uchar)191){
		return;
	}	
...
StringPtr PointerStream::on_get_s(uint_t length){
	...
	return xnew<String>((char*)&data_[saved], (pos_ - saved)/sizeof(char));	
}
...
StringPtr PointerStream::on_get_s_all(){
	...
	StringPtr ret = xnew<String>((char*)&data[pos_], size_ - pos_);
	pos_ = size_;
	return ret;
}
...
const StringPtr& MemoryStream::to_s(){
	str_ = XNew<String>((char*)data_, size_/sizeof(char));
	return str_;
}
...
:str_(str ? str : empty_string){
	data_ = (u8*)str_->data();
	size_ = str_->data_size()*sizeof(char);
	pos_ = 0;
}
