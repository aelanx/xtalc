...
inline bool test_ident_first(int ch){
	return test_alpha((char)ch) || ch_len((char)ch)>1 || ch>255;
}
	...
	for(;;){
		...
		if(ch=='\\'){
			...
			switch(executor_->peek_ascii()){
				XTAL_DEFAULT{ 
					chs[n++] = '\\';
					chs[n++] = (char)executor_->peek_ascii();
				}
			...
			if(ch=='\r'){
				...
				ms_->write(str->data(), str->data_size()*sizeof(char));
			}
		}	
	}
...
void Parser::expect(int_t ach){
	...
	if(ch.type()==Token::TYPE_IDENTIFIER){
		...
		int_t n = ch.ivalue();
		char buf[] = {((n>>0)&0xff), ((n>>8)&0xff), ((n>>16)&0xff), ((n>>24)&0xff), 0};
	...
	char buf2[] = {((ach>>0)&0xff), ((ach>>8)&0xff), ((ach>>16)&0xff), ((ach>>24)&0xff), 0};
