...
char FormatSpecifier::change_int_type(){
	if(is_int_type()){
		return type_;
	}
	return 'd';
...
char FormatSpecifier::change_float_type(){
	if(is_float_type()){
		return type_;
	}
	return 'g';
...
void FormatSpecifier::make_format_specifier(char* dest, char type, bool int_type){
	*dest++ = '%';
...
uint_t FormatSpecifier::parse_format(const char* const str, uint_t i, uint_t sz){
	width_ = 0;
	precision_ = 0;
	pos_ = 0;		
...
uint_t FormatSpecifier::parse_format_digit(const char* const str, uint_t i, uint_t sz, int_t& digit){
	digit = 0;
	while(i!=sz){
		if(str[i]>='0' && str[i]<= '9'){
			digit *= 10;
...
uint_t FormatSpecifier::parse_format_inner(const char* const str, uint_t i, uint_t sz){
...
void String::on_rawcall(const VMachinePtr& vm){
	...
	char cbuf[LIMIT+1];
	char spec[FormatSpecifier::FORMAT_SPECIFIER_MAX];
