...
public:
	...
	WinFindNextFile(const char* path){
		char path2[MAX_PATH];
...
public:
	...
	virtual bool is_directory(const char* path){
	...
	virtual uint_t mtime(const char* path){ 
		XTAL_TC(WIN32_FIND_DATA) fd;
		HANDLE h = XTAL_TC(FindFirstFile)(path, &fd);
		if(h==INVALID_HANDLE_VALUE){
			return 0;
	...
	virtual void* new_file_stream(const char* path, const char* flags){
	...
	virtual void* new_entries(const char* path){
	...
	virtual const char* next_entries(void* entries_object){
