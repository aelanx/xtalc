...
public:
	...
	char change_int_type();
	char change_float_type();
	bool is_int_type();
	bool is_float_type();

	void make_format_specifier(char* dest, char type, bool int_type = false);

	uint_t parse_format(const char* str, uint_t i, uint_t sz);

private:
	
	uint_t parse_format_inner(const char* str, uint_t i, uint_t sz);

	uint_t parse_format_digit(const char* str, uint_t i, uint_t sz, int_t& digit);
};
