...
uint_t string_hashcode(const char* data, uint_t len){
...
void string_data_size_and_hashcode(const char* str, uint_t& size, uint_t& hash){
	size = string_data_size(str);
	hash = string_hashcode(str, size);
}

uint_t string_length(const char* str){
	ChMaker chm;
	uint_t length = 0;
	uint_t i=0;
	while(str[i]){
...
uint_t string_data_size(const char* str){
	uint_t ret = 0;
	while(*str++){
		ret++;
	}
...
int_t string_compare(const char* a, uint_t asize, const char* b, uint_t bsize){
	if(a==b){ return 0; }
	if(asize<bsize){ return -1; }
	if(bsize<asize){ return 1; }

	for(uint_t i=0; i<asize; ++i){
		int_t diff = (int_t)((uchar*)a)[i] - (int_t)((uchar*)b)[i];
		if(diff!=0){
			return diff;
		}
	}
...
void string_copy(char* a, const char* b, uint_t size){
	while(size!=0){
		*a++ = *b++;
		size--;
	}
...
bool string_is_ch(const char* str, uint_t size){
	ChMaker chm;
...
int_t edit_distance(const StringPtr& str1, const StringPtr& str2){
	return edit_distance(str1->data(), str1->data_size()*sizeof(char), str2->data(), str2->data_size()*sizeof(char));
}
...
void String::init_small_string(const char* str, uint_t size){
	value_.init_small_string(str, size);
}

void String::init_string(const char* str, uint_t size){
...
void String::init_long_lived_string(const char* str, uint_t size){
	if(size<SMALL_STRING_MAX){
		init_small_string(str, size);
	}
	else{
...
StringData::StringData(uint_t size){
	...
	data_size_ = size;
	buf_ = (char*)xmalloc(sizeof(char)*(size+1));
	buf()[size] = 0;
}

StringData::~StringData(){
	xfree(buf_, sizeof(char)*(data_size()+1));
}

////////////////////////////////////////////////////////////////

SmartPtr<ID>::SmartPtr(const char* v)
	:Any(ID(v)){}
