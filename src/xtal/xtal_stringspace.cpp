...
void StringSpace::uninitialize(){
	for(uint_t i=0; i<nodes_size_; ++i){
		...
		if(node && node->flags==2){
			xfree(node->pointer(), (node->size+1)*sizeof(char));
		}
	}
...
const char* StringSpace::register_string(const char* str, uint_t size, uint_t hashcode, bool long_lived){
	...
	if(!long_lived || size<8){
		if(size<48){
			p = (Node*)alloc(sizeof(Node) + (size+1)*sizeof(char));
			p->flags = 0;
			newstr = p->buf();
		}
		else{
			p = (Node*)alloc(sizeof(Node) + sizeof(char*));
			p->flags = 2;
			newstr = (char*)xmalloc((size+1)*sizeof(char));
